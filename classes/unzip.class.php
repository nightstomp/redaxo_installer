...
class dUnzip2{
	...
	Function unzip($compressedFileName, $targetFileName=false, $applyChmod=0777){
		...
		if($applyChmod && $targetFileName)
			@chmod($targetFileName, 0777); // Dirty: Fehlermeldung unterdr¸cken, auf manchen System chmod nicht erlaubt
			...
			if(!is_dir($outDN) && $maintainStructure){
				...
				foreach($folders as $folder){
					...
					if(!is_dir("$targetDir/$str")){
						...
						mkdir("$targetDir/$str");
						if($applyChmod)
							chmod("$targetDir/$str", $applyChmod); // Dirty: Fehlermeldung unterdr¸cken, auf manchen System chmod nicht erlaubt
					}
				}
			}
